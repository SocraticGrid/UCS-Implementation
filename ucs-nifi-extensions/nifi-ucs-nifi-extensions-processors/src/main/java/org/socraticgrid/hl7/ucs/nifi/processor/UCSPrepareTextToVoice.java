...
@EventDriven
@Tags({"UCS", "Voice"})
@CapabilityDescription("Given a serialized MessageWrapper as the content of a FlowFile, this processor will extract the require information to send an Text-toVoice message to each of the participants. "
        + "This processor generates as many outgoing FlowFiles as TEXT-TO-VOICE Recipients the original message has.")
public class UCSPrepareTextToVoice extends AbstractProcessor {
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) {
        ...
        for (FlowFile flowFile : flowFiles) {
            ...
            if (recipients.isEmpty()) {
                ...
                //to this processor, it must be because we at least have 1 TEXT-TO-VOICE address.
                logger.error("The message didn't contain any Address with ServiceId '{}'.", new Object[]{context.getProperty(SERVICE_ID).getValue()});
                UCSCreateException.routeFlowFileToException(
                        context,
                        session,
                        logger,
                        ...
                        REL_FAILURE,
                        null,
                        "Error processing Message: The message didn't contain any Address with ServiceId '"+context.getProperty(SERVICE_ID).getValue()+"'.",
            ...
            MessageBody body = MessageBodyResolver.resolveMessagePart(message, context.getProperty(SERVICE_ID).getValue(), firstRecpient.getRecipientId(), firstRecpient.getDeliveryAddress().getPhysicalAddress().getAddress());
            String text = body.getContent();
            
            for (Recipient r : recipients) {
            	MessageBody msgBody = MessageBodyResolver.resolveMessagePart(message, context.getProperty(SERVICE_ID).getValue(), r.getRecipientId(), r.getDeliveryAddress().getPhysicalAddress().getAddress());
            ...
            //if we expect a response from these messages we need to 
