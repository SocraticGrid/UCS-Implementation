 ...
 *
 */
@Tags({"adapters", "UCS", "status", "UCSStatus"})
@CapabilityDescription("This processor gets the over all service status as well as individual Adapter Status")
public class UCSGetServiceStatus extends AbstractProcessor {
    ...
    public static final Relationship REL_SUCCESS = new Relationship.Builder()
            .name("success").description("UCSStatus model which wraps individual adapter Stattus.")
            .build();
        ...
        try {
            UCSController ucsService = context.getProperty(
                    UCS_CONTROLLER_SERVICE).asControllerService(
                            UCSController.class);
            UCSStatus ucsStatus = ucsService.getServiceStatus();
            logger.debug("UCSStatus : " + ucsStatus);
            ...
            flowFile = session.write(flowFile, new OutputStreamCallback() {
                ...
                public void process(OutputStream out) throws IOException {
                    try {
                        out.write(UCSStatusSerializer.serializeUCSStatus(ucsStatus).getBytes());
                    } catch (Exception e) {
                        // should never happen
                        getLogger().error(
            ...
            logger.error("Exception while processing UCSStatus.", e);
            UCSCreateException.routeFlowFileToException(context, session,
                    logger, session.create(), REL_FAILURE, null,
                    "Exception while processing UCSStatus: " + e.getMessage(),
                    ExceptionType.General, null, null);
        }
